<template>
  <main class="flex-1 p-0 overflow-y-auto bg-white relative">
    ...
    <Step3ExecutePrompt v-if="currentStep === 3" @action="handleAction" ref="step3Ref" :initial-git-diff="initialGitDiff" :initial-split-line-limit="initialSplitLineLimit" @update:shotgunGitDiff="(val) => emit('update:shotgunGitDiff', val)" @update:splitLineLimit="(val) => emit('update:splitLineLimit', val)" :final-prompt="props.finalPrompt" :open-router-api-key="props.openRouterApiKey" @update:open-router-api-key="(newKey) => emit('update:openRouterApiKey', newKey)" />
